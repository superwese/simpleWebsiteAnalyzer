...
/**
 * Fetches HttpStatusException and returns a {@link javax.ws.rs.core.Response} Response where an
 * "x-reason" is filled with the exception's message.
 * 
...
public class HttpStatusExceptionMapper implements ExceptionMapper<HttpStatusException> {
  ...
  public Response toResponse(HttpStatusException exception) {
    Status errorStatus = Status.fromStatusCode(exception.getStatusCode());
    String internal_message = errorStatus.getReasonPhrase() + ": " + exception.getUrl();
    Result result = new Result().withResponseStatus(exception.getStatusCode()).withErrormessage(internal_message);
